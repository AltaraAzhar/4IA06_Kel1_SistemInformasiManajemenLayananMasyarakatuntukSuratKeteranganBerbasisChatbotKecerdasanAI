...
@section('title', 'Form Pengajuan - Surat Keterangan Kelahiran')

@section('content')
@php
    $memerlukanEtiket = in_array($layanan['nama'], \App\Models\PengajuanSurat::layananEtiket());
    $showEtiketModal = $memerlukanEtiket;
@endphp
...
<div class="min-h-screen bg-gray-50 py-20">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <!-- Header Layanan (Dark Card with Emoji) -->
        <div class="mb-6">
        ...
        <form id="form-pengajuan" method="POST" action="{{ route('user.pengajuan.store', 'kelahiran') }}" enctype="multipart/form-data">
            @csrf
                ...
                <div id="step-2-content" class="step-content hidden">
                    ...
                    <div class="p-8 space-y-6">
                        ...
                        <div class="border-t pt-6">
                            ...
                            <div class="space-y-4">
                                <!-- Dokumen 1: Surat Pengantar RT/RW -->
                                <div class="border border-gray-300 rounded-lg p-4 hover:border-blue-400 transition-colors">
                                    <div class="flex items-center justify-between">
                                        <div class="flex items-center space-x-3 flex-1">
                                            <i class="fas fa-file-alt text-blue-600 text-xl"></i>
                                            <div class="flex-1">
                                                <p class="text-sm font-semibold text-gray-900">Surat Pengantar RT/RW</p>
                                                <p class="text-xs text-gray-500 mt-1" id="file-name-1">No file chosen</p>
                                            </div>
                                        </div>
                                        <label class="px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white text-sm font-medium rounded-md cursor-pointer transition-colors">
                                            Choose Files
                                            <input type="file" name="doc_pengantar_rt" class="hidden" accept=".pdf,.jpg,.jpeg,.png" onchange="updateFileName(this, 'file-name-1')">
                                        </label>
                                    </div>
                                </div>

                                <!-- Dokumen 2: Fotocopy KK -->
                                <div class="border border-gray-300 rounded-lg p-4 hover:border-blue-400 transition-colors">
                                    <div class="flex items-center justify-between">
                                        <div class="flex items-center space-x-3 flex-1">
                                            <i class="fas fa-file-alt text-blue-600 text-xl"></i>
                                            <div class="flex-1">
                                                <p class="text-sm font-semibold text-gray-900">Fotocopy KK orang tua</p>
                                                <p class="text-xs text-gray-500 mt-1" id="file-name-2">No file chosen</p>
                                            </div>
                                        </div>
                                        <label class="px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white text-sm font-medium rounded-md cursor-pointer transition-colors">
                                            Choose Files
                                            <input type="file" name="doc_kk" class="hidden" accept=".pdf,.jpg,.jpeg,.png" onchange="updateFileName(this, 'file-name-2')">
                                        </label>
                                    </div>
                                </div>

                                <!-- Dokumen 3: Fotocopy KTP -->
                                <div class="border border-gray-300 rounded-lg p-4 hover:border-blue-400 transition-colors">
                                    <div class="flex items-center justify-between">
                                        <div class="flex items-center space-x-3 flex-1">
                                            <i class="fas fa-file-alt text-blue-600 text-xl"></i>
                                            <div class="flex-1">
                                                <p class="text-sm font-semibold text-gray-900">Fotocopy KTP orang tua</p>
                                                <p class="text-xs text-gray-500 mt-1" id="file-name-3">No file chosen</p>
                                            </div>
                                        </div>
                                        <label class="px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white text-sm font-medium rounded-md cursor-pointer transition-colors">
                                            Choose Files
                                            <input type="file" name="doc_ktp" class="hidden" accept=".pdf,.jpg,.jpeg,.png" onchange="updateFileName(this, 'file-name-3')">
                                        </label>
                                    </div>
                                </div>

                                <!-- Dokumen 4: Buku Nikah -->
                                <div class="border border-gray-300 rounded-lg p-4 hover:border-blue-400 transition-colors">
                                    <div class="flex items-center justify-between">
                                        <div class="flex items-center space-x-3 flex-1">
                                            <i class="fas fa-file-alt text-blue-600 text-xl"></i>
                                            <div class="flex-1">
                                                <p class="text-sm font-semibold text-gray-900">Fotocopy Buku Nikah / Akta Perkawinan</p>
                                                <p class="text-xs text-gray-500 mt-1" id="file-name-4">No file chosen</p>
                                            </div>
                                        </div>
                                        <label class="px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white text-sm font-medium rounded-md cursor-pointer transition-colors">
                                            Choose Files
                                            <input type="file" name="doc_nikah" class="hidden" accept=".pdf,.jpg,.jpeg,.png" onchange="updateFileName(this, 'file-name-4')">
                                        </label>
                                    </div>
                                </div>

                                <!-- Dokumen 5: Surat Keterangan Lahir -->
                                <div class="border-2 border-purple-300 bg-purple-50 rounded-lg p-4 hover:border-purple-400 transition-colors">
                                    <div class="flex items-center justify-between">
                                        <div class="flex items-center space-x-3 flex-1">
                                            <i class="fas fa-file-alt text-purple-600 text-xl"></i>
                                            <div class="flex-1">
                                                <p class="text-sm font-semibold text-gray-900">Asli dan fotocopy Surat Keterangan Kelahiran dari Rumah Sakit / Bidan</p>
                                                <p class="text-xs text-gray-500 mt-1" id="file-name-5">No file chosen</p>
                                            </div>
                                        </div>
                                        <label class="px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white text-sm font-medium rounded-md cursor-pointer transition-colors">
                                            Choose Files
                                            <input type="file" name="doc_surat_lahir" class="hidden" accept=".pdf,.jpg,.jpeg,.png" onchange="updateFileName(this, 'file-name-5')">
                                        </label>
                                    </div>
                                </div>
                            </div>
...
<script>
@if($showEtiketModal)
// Show e-Ticket modal on page load
document.addEventListener('DOMContentLoaded', function() {
    const modal = document.getElementById('etiket-modal');
    const formContainer = document.querySelector('.min-h-screen.bg-gray-50');
